...
El modulo CLM tiene tests automatizados (Jest + Supertest) que prueban el
login, el CRUD completo de las 3 tablas, y las reglas de negocio que viven
en los triggers de Postgres (la maquina de estados de `contrato` y el
bloqueo optimista de `clausula`).

**Los tests corren contra una base de datos SEPARADA de la que usas para tu
demo** (`clm_db_test` en vez de `clm_db`), porque cada test vacia las tablas
antes de correr (`TRUNCATE`) — si apuntaran a tu base real, borrarian tus
datos de verdad en cada ejecucion.

Configuracion (una sola vez):

1. Crea una base de datos nueva, vacia, llamada `clm_db_test` (mismo proceso
   que usaste para `clm_db` en el paso 1 de arriba: pgAdmin o psql, y corre
   el mismo script `3_tablas_principales.sql` contra ella — no hace falta
   cargarle `seed_datos_ejemplo.sql`, los tests generan sus propios datos).
2. Copia el archivo de configuracion de tests:
   ```
   copy .env.test.example .env.test
   ```
3. Abre `.env.test` y completa `PGPASSWORD` con la contraseña real de tu
   Postgres (igual que en tu `.env` normal). Las demas variables ya vienen
   listas para funcionar sin tocarlas.

Para correr los tests:

```bash
npm install   # trae jest y supertest (devDependencies)
npm test
```

Deberias ver algo como `Tests: 29 passed, 29 total`. Si un test falla,
Jest te muestra exactamente que esperaba vs que recibio — es la forma mas
rapida de darte cuenta si un cambio que hiciste rompio algo (por ejemplo,
si tocas una validacion o una consulta SQL y sin querer cambias su
comportamiento).

Los archivos de test estan en `tests/`, uno por tabla mas uno de
autenticacion (`auth.test.js`, `empresas.test.js`, `contratos.test.js`,
`clausulas.test.js`), y comparten dos ayudantes en `tests/helpers/`:
`authAgent.js` (inicia sesion y devuelve un cliente HTTP que ya trae la
cookie puesta) y `resetDb.js` (vacia las 3 tablas antes de cada test, para
que ninguno dependa de datos que dejo otro test anterior).
