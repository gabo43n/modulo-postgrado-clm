...
export default function EditClausulaScreen({ route, navigation }) {
  const { clausula } = route.params;
  const [form, setForm] = useState({
    id_contrato: clausula.id_contrato,
    orden: clausula.orden.toString(),
    titulo: clausula.titulo,
    contenido: clausula.contenido || '',
    tipo_clausula: clausula.tipo_clausula || 'estandar',
    es_modificable: clausula.es_modificable !== undefined ? clausula.es_modificable : true
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await clmService.updateClausula(clausula.id_clausula, data);
      Alert.alert('Éxito', 'Cláusula actualizada correctamente');
      navigation.goBack();
    } catch (e) {
      console.error(e);
      Alert.alert('Error', 'No se pudo actualizar la cláusula. Verifique si la cláusula está bloqueada.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        <ScrollView style={styles.container}>
          <Text style={styles.title}>Editar Cláusula</Text>
          <View style={styles.infoBox}>
            <Text style={styles.infoText}>Contrato ID: {clausula.id_contrato}</Text>
          </View>
          ...
          <TouchableOpacity
            ...
            disabled={loading}
          >
            {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.buttonText}>Actualizar Cláusula</Text>}
          </TouchableOpacity>
